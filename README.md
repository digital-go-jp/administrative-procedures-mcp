...
- **プラットフォーム限定** — Prompt API は Chrome（138+。151 で確認）と Microsoft Edge Canary/Dev（138.0.3309.2+。`edge://flags` の「Prompt API for on-device language model」を有効化）で利用可能です。ただし起動時のデータセット探索までは任意のブラウザで利用可能です。
- **進行状況の表示** — 質問中は、セッション準備・ツール選択・引数生成・実行の各ステップと経過秒、生成中のモデル出力を画面に表示します。応答が止まったときは「中止」ボタンで打ち切れます。
